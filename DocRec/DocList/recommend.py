...
def Top10_Docs(docs_df, id):
    ...
    rank = similarity_rank[1:11]

    Top10 = pd.DataFrame(columns=['title', 'url', 'content'])
    for idx in rank:
        # print (docs_df.iloc[idx[0]])
        doc = docs_df.iloc[idx[0]]
        # doc['id'] = idx[0]
        Top10 = Top10.append(doc)

    return Top10["title"].tolist(), frequency
